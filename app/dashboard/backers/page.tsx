...
export default async function BackersPage() {
  ...
  const title =
    session.role === "super_admin"
      ? "Campaign backers"
      : session.role === "donor_individual" || session.role === "donor_business"
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Paid campaign donation activity connected to campaign backer records, receipts, and donor visibility settings.
          </p>
        </div>
      </div>
