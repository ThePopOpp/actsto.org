...
export default async function AboutUsPage() {
  const [heroCta, midCta, bottomCta] = await Promise.all([
    getCtaBlockByPlacement("about_hero"),
    getCtaBlockByPlacement("about_mid"),
    getCtaBlockByPlacement("about_bottom"),
  ]);
      ...
      <section className="border-b border-border/60 bg-gradient-to-b from-act-banner/40 to-background dark:from-act-banner/10">
        <div className="mx-auto max-w-3xl px-4 py-14 text-center sm:px-6 lg:px-8 lg:py-20">
          <h1 className="font-heading text-4xl font-semibold tracking-tight text-primary sm:text-5xl">
            {heroCta?.heading ?? "About Arizona Christian Tuition"}
          </h1>
          <p className="mx-auto mt-6 text-lg leading-relaxed text-muted-foreground">
            {heroCta?.body ??
              "Arizona Christian Tuition (ACT) was founded by three Arizona dads who shared a common challenge and a shared conviction. As fathers, we wanted to help families access private Christian education with clarity, confidence, and practical support."}
          </p>
          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            {heroCta?.showSecondary !== false ? (
              ...
              >
                {heroCta?.secondaryLabel ?? "Support a campaign"}
              </Link>
            ) : null}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="mt-4 font-heading text-2xl font-semibold text-primary sm:text-3xl">
              {midCta?.heading ?? "Join families, schools, and donors making tuition possible"}
            </h2>
            ...
            <div className="mt-8 flex w-full max-w-md flex-col gap-3 sm:max-w-none sm:flex-row sm:justify-center">
              <Link href={midCta?.primaryUrl ?? "/contact"} className={cn(buttonVariants({ size: "lg" }), "w-full sm:w-auto")}>
                {midCta?.primaryLabel ?? "Talk to our team"}
              </Link>
              {midCta?.showSecondary !== false ? (
                <Link
                  href={midCta?.secondaryUrl ?? "/faq"}
                  className={cn(
                    buttonVariants({ variant: "outline", size: "lg" }),
                    "w-full border-primary bg-background sm:w-auto"
                  )}
                >
                  {midCta?.secondaryLabel ?? "Read the FAQ"}
                </Link>
              ) : null}
            </div>
          </div>
      ...
      <section className="bg-primary py-14 text-primary-foreground sm:py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <h2 className="font-heading text-2xl font-semibold sm:text-3xl">
            {bottomCta?.heading ?? "Ready to put your taxes to work for Christian education?"}
          </h2>
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href={bottomCta?.primaryUrl ?? "/register"}
              className={cn(
                buttonVariants({ variant: "secondary", size: "lg" }),
                "min-w-[180px] bg-background text-primary hover:bg-background/90"
              )}
            >
              {bottomCta?.primaryLabel ?? "Get started free"}
            </Link>
            {bottomCta?.showSecondary !== false ? (
              <Link
                href={bottomCta?.secondaryUrl ?? "/campaigns"}
                className={cn(
                  buttonVariants({ variant: "outline", size: "lg" }),
                  "min-w-[180px] border-primary-foreground/40 bg-transparent text-primary-foreground hover:bg-primary-foreground/10"
                )}
              >
                {bottomCta?.secondaryLabel ?? "Browse campaigns"}
              </Link>
            ) : null}
          </div>
        </div>
